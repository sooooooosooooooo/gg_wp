...
namespace gg_wp
{
    internal class Map
    {
        ...
        public int Health_potion_count {  get; protected set; }
        public Random rnd = new Random();
        public Map(int x,int y,int enemy_count,int health_potion_count)         // set length,width of map and count of enemy and health potions (constructor)
        {
            ...
            Enemy_count = enemy_count < 3 ? 3 : enemy_count;                            
            Health_potion_count = health_potion_count < 3 ? 3 : health_potion_count;
        }
        public char[,] CreateMap()                                              //creating map with information which was dated in constructor
        {
            ...
            for (int i = 0;i < Enemy_count; i++)                                //randoming enemy in random positions on the map
            {
                int x_path = rnd.Next(0, Map.GetLength(0));
                int y_path = rnd.Next(0, Map.GetLength(1));
                if (Map[x_path, y_path] != '#' && Map[x_path, y_path] != '@' && Map[x_path, y_path] != '*')
                {
                    Map[x_path, y_path] = '*';
                }
                else i--;
            }
            for (int i = 0;i < Health_potion_count; i++)                         //randoming potion location on the map
            {
                int x_path = rnd.Next(0, Map.GetLength(0));
                int y_path = rnd.Next(0, Map.GetLength(1));
                if (Map[x_path, y_path] != '#' && Map[x_path, y_path] != '@' && Map[x_path, y_path] != '*' && Map[x_path,y_path] != '&')
                {
                    Map[x_path, y_path] = '&';
                }
                else i--;
